  ...
  <AppLayout>
    ...
    <div class="bg-[#111] border-b border-white/5 pt-5 pb-4 px-4">
      <div class="max-w-7xl mx-auto">
        <h1 class="text-2xl sm:text-3xl md:text-4xl font-black text-[#e91e8c] mb-4">
        ...
        <div v-if="profiles.data.length" class="relative mb-4">
          <div class="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            <a v-for="profile in profiles.data.slice(0,12)" :key="profile.id"
              ...
              <div class="w-14 h-14 sm:w-[72px] sm:h-[72px] rounded-full overflow-hidden ring-2 ring-[#e91e8c] ring-offset-2 ring-offset-[#111] group-hover:ring-[#ff6abb] transition">
              ...
              <span class="text-xs text-gray-400 mt-1 text-center truncate w-full group-hover:text-[#e91e8c] transition">{{ profile.display_name }}</span>
            </a>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 sm:gap-3 mb-3">
          <div class="relative">
            <select v-model="filters.city"
              class="w-full bg-[#1a1a1a] border border-white/10 text-gray-200 text-sm rounded px-3 py-2.5 pr-8 appearance-none cursor-pointer hover:border-[#e91e8c] transition sm:min-w-[130px] focus:outline-none focus:border-[#e91e8c]">
          ...
          <div class="relative col-span-2 sm:col-span-1 sm:flex-1">
            <input v-model="filters.search" type="text" placeholder="Suchen..."
              ...
              class="w-full bg-[#1a1a1a] border border-white/10 text-gray-200 text-sm rounded px-4 py-2.5 pr-10 focus:outline-none focus:border-[#e91e8c] placeholder-gray-600" />
    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-4 pb-12 pt-4 sm:pt-6">
      ...
      <div v-else class="space-y-3">
        ...
        <a v-if="profiles.data[0]" :href="route('profile.show', profiles.data[0].slug)"
          class="block bg-[#1a1a1a] border border-white/8 rounded-xl overflow-hidden hover:border-[#e91e8c]/60 transition group shadow-xl">
          <div class="flex">
            ...
            <div class="p-3 sm:p-5 flex-1 min-w-0">
              <h2 class="text-base sm:text-xl font-bold text-white group-hover:text-[#e91e8c] transition mb-1 sm:mb-2 truncate">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <a v-for="profile in profiles.data.slice(1)" :key="profile.id"
            :href="route('profile.show', profile.slug)"
            class="flex bg-[#1a1a1a] border border-white/8 rounded-xl overflow-hidden hover:border-[#e91e8c]/60 transition group shadow-lg"
      ...
      <div v-if="profiles.last_page > 1" class="flex justify-center flex-wrap gap-2 mt-8">
        <Link v-for="link in profiles.links" :key="link.label"
          ...
          :class="link.active
            ...
            : 'bg-[#1a1a1a] text-gray-400 border-white/10 hover:border-[#e91e8c]/50 hover:text-[#e91e8c]'" />
      </div>
    </div>
  </AppLayout>
</template>
