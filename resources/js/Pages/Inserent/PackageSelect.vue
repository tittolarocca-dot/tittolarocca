<template>
  <AppLayout>
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      <div class="flex items-center gap-0 mb-8">
        <div v-for="(step, i) in steps" :key="i" class="flex items-center gap-0 flex-1 last:flex-none">
          <div :class="[
            ...
            i <= 1 ? 'bg-pink-600 text-white' : 'bg-gray-200 text-gray-500',
          ]">{{ i < 1 ? '✓' : i + 1 }}</div>
          <span class="ml-1.5 text-xs font-medium"
            :class="i === 1 ? 'text-pink-600' : i < 1 ? 'text-green-600' : 'text-gray-400'">
            {{ step }}
          </span>
          <div v-if="i < steps.length - 1" class="flex-1 h-px mx-3"
            :class="i < 1 ? 'bg-pink-400' : 'bg-gray-200'"></div>
        </div>
      </div>

      <h1 class="text-2xl font-bold text-gray-800 mb-2">Paket wählen</h1>
      <p class="text-gray-500 text-sm mb-8">
        Wähle ein Laufzeit-Paket für <strong class="text-gray-700">„{{ profile.display_name }}"</strong>.
        Nach erfolgreicher Zahlung ist dein Inserat <span class="text-pink-600 font-semibold">sofort aktiv</span>.
      </p>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div v-for="pkg in packages" :key="pkg.id"
          ...
          <div v-if="pkg.name === 'Standard'"
            class="absolute -top-3 left-1/2 -translate-x-1/2 bg-pink-600 text-white text-xs font-bold px-3 py-1 rounded-full whitespace-nowrap">
            Beliebteste Wahl
          </div>
          ...
          <div :class="[
            ...
            selectedId === pkg.id ? 'border-pink-500 bg-pink-500' : 'border-gray-300',
          ]">
          ...
          <div class="mt-1 mb-3">
            <span class="text-2xl font-extrabold text-pink-600">CHF {{ pkg.price_chf }}</span>
      ...
      <!-- Preisdetail + Checkout -->
      <div v-if="selected" class="bg-white rounded-2xl border border-gray-200 p-6">
        <div class="flex items-center justify-between flex-wrap gap-4">
          <div>
            <p class="font-semibold text-gray-800">{{ selected.name }}-Paket</p>
            <p class="text-sm text-gray-500">{{ selected.duration_days }} Tage Laufzeit · automatisch deaktiviert nach Ablauf</p>
            <p class="text-xs text-gray-400 mt-1">Sichere Zahlung via Stripe · Kreditkarte, TWINT (bald)</p>
          </div>
          <div class="text-right">
            <p class="text-3xl font-extrabold text-pink-600">CHF {{ selected.price_chf }}</p>
