  ...
  <AppLayout>
    ...
    <div class="bg-[#0f0f0f]">
      <div class="max-w-7xl mx-auto">
        ...
        <div class="sm:hidden">
          <div v-if="publicMedia.length" class="relative">
            <div class="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide" ref="mobileCarousel" @scroll="onCarouselScroll">
              <div
                v-for="(item, i) in publicMedia"
                :key="item.id"
                class="shrink-0 w-full snap-center aspect-[4/5] relative overflow-hidden cursor-pointer"
            ...
            <!-- Dot indicators -->
            <div v-if="publicMedia.length > 1" class="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5 pointer-events-none">
              <span
                v-for="(item, i) in publicMedia.slice(0, 12)"
                :key="i"
                class="w-2 h-2 rounded-full transition-all duration-200"
                :class="i === activeCarouselIndex ? 'bg-[#e35d8f] scale-125' : 'bg-white/40'" />
            </div>
          </div>
          <div v-else class="aspect-[4/5] bg-white/5 flex items-center justify-center text-gray-500 text-6xl">👤</div>
        </div>
        ...
        <div class="hidden sm:block">
          ...
          <!-- No images -->
          <div v-if="!publicMedia.length" class="h-[500px] bg-white/5 flex items-center justify-center text-gray-500 text-6xl">👤</div>

          <!-- 1 image: full-width hero -->
          <div v-else-if="publicMedia.length === 1"
            class="relative h-[420px] md:h-[540px] overflow-hidden cursor-pointer group"
            ...
            <div class="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-black/70 to-transparent pointer-events-none" />
          </div>

          <!-- 2 images: two equal columns -->
          <div v-else-if="publicMedia.length === 2"
            class="grid grid-cols-2 gap-1 h-[420px] md:h-[540px]">
            <div
              v-for="(item, i) in publicMedia.slice(0, 2)"
              :key="item.id"
              class="relative overflow-hidden cursor-pointer group"
              @click="openLightbox(item)">
              <img
                :src="i === 0 ? item.url : undefined"
                :data-src="i > 0 ? item.url : undefined"
                :class="['w-full h-full object-cover object-center transition duration-300 group-hover:scale-105', i > 0 ? 'lazyload' : '']"
                :alt="profile.display_name"
          ...
          <!-- 3–4 images: big left (2/3) + stacked right (1/3) -->
          <div v-else-if="publicMedia.length <= 4"
            class="flex gap-1 h-[420px] md:h-[540px]">
            ...
            <div class="flex flex-col gap-1 flex-1">
              <div
                v-for="(item, i) in publicMedia.slice(1)"
                :key="item.id"
                class="relative flex-1 overflow-hidden cursor-pointer group"
                @click="openLightbox(item)">
                <img
                  class="lazyload w-full h-full object-cover object-center transition duration-300 group-hover:scale-105"
                  :data-src="item.url"
                  src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                  :alt="profile.display_name" />
              </div>
          ...
          <!-- 5+ images: big left (3/5) + 2×2 grid right (2/5) -->
          <div v-else class="flex gap-1 h-[420px] md:h-[540px]">
            ...
            <div class="grid grid-cols-2 gap-1" style="flex:2">
              <div
                v-for="(item, i) in publicMedia.slice(1, 5)"
                :key="item.id"
                class="relative overflow-hidden cursor-pointer group"
                @click="openLightbox(item)">
                <img
