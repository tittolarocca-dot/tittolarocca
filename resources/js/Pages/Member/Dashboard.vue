    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <Link v-for="action in actions" :key="action.label" :href="action.href"
          class="bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-5 flex items-center gap-4 hover:border-[#e91e8c]/40 transition group">
        ...
        <div v-else class="space-y-3">
          <div v-for="sub in subscriptions.slice(0,3)" :key="sub.id"
            ...
            <Link :href="route('profile.show', sub.profile.slug)"
              class="font-semibold text-white hover:text-[#e91e8c] transition text-sm">
              {{ sub.profile.display_name }}
            </Link>
            <div class="text-right">
              <p class="text-xs font-bold text-white">CHF {{ sub.amount_chf }}/Mo</p>
              <span class="text-xs" :class="sub.status === 'active' ? 'text-green-500' : 'text-yellow-500'">
      ...
      <div class="mt-6 bg-[#111111] rounded-xl border border-[#2a2a2a] p-5">
        <h3 class="font-semibold text-white text-sm mb-3">So funktioniert es</h3>
        <div class="grid sm:grid-cols-3 gap-4 text-xs text-gray-400">
          <div class="flex gap-3">
            <span class="text-xl shrink-0">🔍</span>
            <div>
              <p class="font-semibold text-gray-200 mb-0.5">Profile kostenlos ansehen</p>
          ...
          <div class="flex gap-3">
            <span class="text-xl shrink-0">🔒</span>
            <div>
              <p class="font-semibold text-gray-200 mb-0.5">Exklusive Inhalte freischalten</p>
          ...
          <div class="flex gap-3">
            <span class="text-xl shrink-0">💬</span>
            <div>
              <p class="font-semibold text-gray-200 mb-0.5">Direkt Schreiben</p>
